...
const seedData = async () => {
  try {
    console.log('--- Starting SmartApply Database Seeder ---');
    await connectDB();
    ...
    // 4. Create Demo Admin and Demo Student
    ...
    // Student Profile for Aarav Patel
    ...
    // 6. Create Primary Active Application for Aarav Patel
    const aaravApp = await Application.create({
      applicationNumber: 'SA-2026-88102',
      studentId: studentUser._id,
      collegeId: colleges[0]._id,
      courseId: courses[0]._id, // B.Tech AI & DS
    ...
    // Initial Notifications for Aarav
    ...
    // 7. Create 6 additional diverse students and applications for rich demo metrics
    const sampleStudentsData = [
      {
        ...
        tenthPct: 93.0,
        entranceScore: 92.5,
        remarks: 'Direct merit seat offered. Outstanding entrance performance.'
      },
      {
        name: 'Rahul Verma',
        email: 'rahul@student.edu',
        phone: '+91 97110 55443',
        courseIndex: 2, // Robotics
        status: 'ACADEMIC_REVIEW',
        stream: 'Science (PCM)',
        twelfthPct: 81.5,
        tenthPct: 84.0,
        entranceScore: 72.0,
        remarks: 'Portfolio under review by Mechatronics Department board.'
      },
      {
        name: 'Ananya Roy',
        email: 'ananya@student.edu',
        phone: '+91 99001 22334',
        courseIndex: 3, // Data Analytics
        status: 'WAITLISTED',
        stream: 'Commerce',
        twelfthPct: 76.0,
        tenthPct: 79.5,
        entranceScore: 61.0,
        remarks: 'Waitlist position #4 for Category General.'
      },
      {
        name: 'Vikram Singh',
        email: 'vikram@student.edu',
        phone: '+91 98440 66778',
        courseIndex: 4, // B.Com FinTech
        status: 'CORRECTION_REQUIRED',
        stream: 'Commerce',
        twelfthPct: 79.0,
        tenthPct: 82.0,
        entranceScore: 55.0,
        remarks: 'Transfer certificate signature blurred. Re-upload requested.'
      },
      {
        name: 'Sneha Gupta',
        ...
        tenthPct: 89.0,
        entranceScore: 84.0,
        remarks: 'Admission confirmed and semester tuition fees deposited.'
      },
      {
        name: 'Devendra Kumar',
        email: 'devendra@student.edu',
        ...
        tenthPct: 68.0,
        entranceScore: 48.0,
        remarks: 'Did not satisfy minimum cutoff requirements for 2026 session.'
      }
    ];

    for (let i = 0; i < sampleStudentsData.length; i++) {
      const s = sampleStudentsData[i];
      ...
      await StudentProfile.create({
        ...
        personalInfo: {
          ...
          city: 'Mumbai',
          state: 'Maharashtra',
          annualFamilyIncome: 450000 + i * 50000,
          category: i % 3 === 0 ? 'OBC' : 'General'
        },
        academicInfo: {
          tenth: { board: 'CBSE', passingYear: 2022, percentage: s.tenthPct },
          twelfth: { board: 'CBSE', passingYear: 2024, percentage: s.twelfthPct, stream: s.stream, subjects: [] },
          entranceExams: [{ examName: 'Entrance Test', score: s.entranceScore, year: 2024 }]
        },
        skills: ['Problem Solving', 'Data Analysis'],
        completionPercentage: 85
      });

      const targetCourse = courses[s.courseIndex];
      const appNumber = `SA-2026-${90000 + i}`;

      const sampleApp = await Application.create({
        applicationNumber: appNumber,
        studentId: student._id,
        collegeId: targetCourse.collegeId,
        courseId: targetCourse._id,
        status: s.status,
        academicSnapshot: {
          twelfthPercentage: s.twelfthPct,
          tenthPercentage: s.tenthPct,
          entranceScore: s.entranceScore,
          stream: s.stream
        },
        adminRemarks: s.remarks,
    ...
    console.log(`Created demo student & admin accounts, plus 6 sample applications across all pipeline stages.`);
    console.log('----------------------------------------------------');
    console.log('✅ SmartApply Database Seeding Completed Successfully!');
    console.log('----------------------------------------------------');
    console.log('🔑 DEMO ACCOUNTS:');
    console.log('   Admin:   admin@smartapply.edu  /  Admin@123');
    console.log('   Student: student@smartapply.edu  /  Student@123');
    console.log('----------------------------------------------------');
